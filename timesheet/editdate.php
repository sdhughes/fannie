...
if (isset($_POST['submitted'])) { // If the form has been submitted.
            ...
            if (empty($errors)) { // All good.
                ...
                for ($i = 1; $i <= $entrycount; $i++) {
                    if (is_numeric($ID[$i])) {
                        ...
                        $result = mysqli_query($db_master, $query);
                        if ($result) {$successcount++;} else {echo '<p>Query: ' . $query . '</p><p>MySQL Error: ' . mysqli_error($db_master) . '</p>';}
                    } elseif ($ID[$i] == 'insert') {
        ...
        if ($datediff > 1) { // Bad.
            ...
            $lunchR = mysqli_query($db_master, $lunchQ);
            list($lunch, $lunchID) = mysqli_fetch_row($lunchR);
            ...
            for ($i = 1; $i <= $max; $i++) {
                ...
                $shiftQ = "SELECT * FROM is4c_log.shifts WHERE visible=true ORDER BY ShiftOrder ASC";
