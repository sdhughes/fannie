...
<?php
    ...
    $reportData = array();
    $totalSales = getGrossSalesInRange($date1,$date2);
...
*/
	...
	//Total Sales are tallied from common queries in another file.
	$report .= sprintf('<td>Total Store Sales:</td><td align="right">$%s </td><td></td><td></td><td></td><td></td><td></td></table>', number_format($totalSales,2));
...
/*
	...
	$table_header = "<thead>
		<tr>
		    ...
		    <th>Department</th>
		    <th>Subdepartment</th>
		    <th class=\"{sorter: 'fancyNumber'}\" >Qty</th>
		    <th>Sales</th>
		    <th>Scale</th>
		</tr>
	    </thead>";
...
function drawForm($msg = NULL, $_POST = NULL) {
   echo ' 
	<div class="directions">This report will only show sales figures by Department, not broken down by SKU.<br/>The other report has been renamed appropriately and can be found under <a href="../reports/dept_movement.php" >Department Movement</a> in the navigation menu.</div>
'; 
...
printf ('</div>
		...
		<tr>
		    <td align="left"><input type="checkbox" name="inUse" checked="CHECKED" /> Only "In Use"?</td>
		    <td colspan="2" align="left"><input type="checkbox" name="groupBy" checked="CHECKED" /> Seperate by Sale Price? </td>
		</tr>
                <tr>
  ...
  //echo '<td align="left"><input type="checkbox" name="deptDetails" checked="CHECKED" /> Department details?</td>';
